...
const loginUser = async (payload: Record<string, unknown>) => {
    ...
    const { password, created_at, updated_at, ...user } = data;
    ...
    const token = jwt.sign({ name: data.name, email: data.email, phone: data.phone, role: data.role }, config.jwtSecret as string, {
        expiresIn: "7d",
    });
